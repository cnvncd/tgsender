...
class BroadcastConfig:
    ...
    def __init__(self):
        ...
        self.TARGET_MODE = os.getenv("TARGET_MODE", "all").lower()  # archived|normal|all
        ...
        try:
            ...
            self.MAX_MESSAGE_DELAY = max(1.0, float(os.getenv("MAX_MESSAGE_DELAY", "30")))
        except (ValueError, TypeError):
            raise ConfigurationError("MESSAGE_DELAY and MAX_MESSAGE_DELAY must be valid numbers")
        ...
        if missing_fields:
            raise ConfigurationError(f"Missing required environment variables: {', '.join(missing_fields)}")
        
        # Validate target mode
        if self.TARGET_MODE not in {"archived", "normal", "all"}:
            raise ConfigurationError(f"Invalid TARGET_MODE: {self.TARGET_MODE}. Must be 'archived', 'normal', or 'all'")
...
def send_bot_notification(bot_token: str, chat_id: str, text: str, disable_notification: bool = False) -> bool:
...
async def collect_user_targets(client: TelegramClient, mode: str) -> List[Tuple[int, int]]:
    """
    ...
    try:
        ...
        logging.info(f"Processed {processed_count} dialogs, found {len(targets)} valid targets for mode '{mode}'")
        return targets
...
class BroadcastEngine:
    ...
    async def send_status_notification(self, message: str, disable_notification: bool = False) -> None:
        ...
        try:
            ...
            for i, (user_id, access_hash) in enumerate(targets, 1):
                try:
                    ...
                    if self.config.ADMIN_PROGRESS_CHAT_ID and self.sent_count > 0 and self.sent_count % 500 == 0:
                        await self.send_status_notification(
                            f"📈 Progress: <b>{self.sent_count}</b> sent, <b>{self.fail_count}</b> failed\n"
                            f"Account: <code>{self.config.ACCOUNT_FRIENDLY_NAME or 'Unknown'}</code>",
            ...
            print(f"[RESULT] sent={self.sent_count} failed={self.fail_count} skipped={self.skip_count} total={len(targets)}")

            self.logger.info(f"Broadcast completed: sent={self.sent_count}, failed={self.fail_count}, skipped={self.skip_count}")
            return True
    ...
    async def process_target(self, user_id: int, access_hash: int, message_text: str) -> None:
        ...
        try:
            ...
            self.current_delay = min(self.current_delay * 1.5, self.config.MAX_MESSAGE_DELAY)
            
            # Don't mark as failed, will retry later
            
        except (errors.UserPrivacyRestrictedError, errors.UserDeactivatedError, errors.UserDeactivatedBanError) as e:
            ...
            self.fail_count += 1
            self.logger.debug(f"Invalid user {user_id}: {e}")
