...
async def handle_error(message: types.Message, error: Exception, context: str = "") -> None:
...
def clear_account_history(account: str) -> None:
    ...
    for path in [get_account_history_file(account), 
...
def get_confirm_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="✅ Подтвердить"), KeyboardButton(text="✏️ Изменить текст")],
...
class ProcessManager:
    ...
    @staticmethod
    async def start_broadcast(account: str, data: Dict[str, Any], user_id: int) -> subprocess.Popen:
        ...
        try:
            ...
            env.update({
                ...
                # ВАЖНО: НЕ пробрасываем BOT_TOKEN / BOT_CHAT_ID / ADMIN_PROGRESS_CHAT_ID
                "ACCOUNT_FRIENDLY_NAME": get_account_display_name(account),
            })
            
            # Если очень нужно — можно пробросить только служебный чат админа,
            # но лучше вообще не слать из messages.py
            # if ADMIN_IDS and str(ADMIN_IDS[0]) != str(user_id):
            #     env["ADMIN_PROGRESS_CHAT_ID"] = str(ADMIN_IDS[0])
    ...
    @staticmethod
    async def wait(proc: subprocess.Popen, timeout: int = TIMEOUT_PROCESS) -> Tuple[str, str, int]:
...
async def confirm_broadcast(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        asyncio.create_task(monitor_broadcast_completion(proc, message.from_user.id, data))
...
async def monitor_broadcast_completion(proc: subprocess.Popen, user_id: int, broadcast_data: Dict[str, Any]):
    """Monitor broadcast process completion"""
    account = broadcast_data["account"]
    
    try:
        stdout, stderr, return_code = await ProcessManager.wait(proc, timeout=TIMEOUT_PROCESS)
        ...
        for line in stdout.splitlines():
            ...
            # Новый приоритетный формат (машиночитаемый)
            if line_stripped.startswith("[RESULT]"):
                # ожидаем пары key=value
            ...
            # Старые эвристики — оставляем как запасной вариант
            if "[OK] Sent" in line_stripped or "Message sent successfully" in line_stripped:
                sent_count += 1
            elif "[SKIP]" in line_stripped or "[FAIL]" in line_stripped or "failed" in line_stripped.lower():
                failed_count += 1
            elif "Total users:" in line_stripped or "total" in line_stripped.lower():
                try:
...
async def show_statistics(message: types.Message):
    ...
    if not stats["broadcasts"]:
        await message.answer(
            "📊 Статистика пуста. Рассылок ещё не было.",
            reply_markup=get_main_menu()
        )
        return
...
@admin_required
async def handle_delete_confirmation_response(message: types.Message, state: FSMContext):
...
async def handle_rename_save(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if save_account_names(names):
            ...
            logger.info(f"Account {account_name} renamed from '{old_name}' to '{new_name}' by user {message.from_user.id}")
        else:
            await message.answer(
                "❌ Не удалось сохранить новое имя.",
                reply_markup=get_main_menu()
            )
            ...
            try:
                if await client.is_user_authorized():
                    ...
                    if me:
                        ...
                        session_file_path = os.path.join(SESSION_DIR, f"{account_name}.session")
                        ...
                        status_message = "❌ Не удалось получить информацию о пользователе"
                else:
                    status_message = (
...
async def add_account_phone(message: types.Message, state: FSMContext):
    ...
    if os.path.exists(session_path):
        ...
        await state.update_data(phone=phone, session_name=session_name, overwrite_pending=True)
        return
...
async def start_telethon_auth(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"❌ Ошибка конфигурации: {e}", reply_markup=get_main_menu())
        await state.clear()
        return
    ...
    try:
        async with telethon_client(session_path, api_id, api_hash) as client:
            ...
            if await client.is_user_authorized():
                ...
                if me:
                    ...
                    display_name = ' '.join(part for part in name_parts if part).strip() or phone
        ...
        await message.answer(f"⏰ Слишком много попыток. Подождите {wait_seconds} секунд.")
    except asyncio.TimeoutError:
        await message.answer("⏰ Таймаут подключения к Telegram API.")
    except Exception as e:
...
async def add_account_code(message: types.Message, state: FSMContext):
    ...
    if not is_valid:
        await message.answer(f"❌ {clean_code}")  # Error message is in second return value
        return
        ...
        async with telethon_client(session_path, api_id, api_hash) as client:
            ...
            try:
                ...
                if me:
                    ...
                    logger.info(f"Account {session_name} added successfully by user {message.from_user.id}")
                    await state.clear()
                else:
        ...
        async with telethon_client(session_path, api_id, api_hash) as client:
            ...
            try:
                ...
                if me:
                    ...
                    logger.info(f"Account {session_name} added with 2FA by user {message.from_user.id}")
                    await state.clear()
                else:
                    await message.answer("❌ Не удалось получить информацию о пользователе.")
...
@dp.message(lambda message: message.text in ["✅ Да, перезаписать", "❌ Нет, отменить"])
@admin_required
